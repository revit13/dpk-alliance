...
from data_processing.examples.noop.python import (
    ...
    NOOP4PythonTransformConfiguration
)
...
class PipelineForkDemo2PythonTransformConfiguration(PythonTransformRuntimeConfiguration):
    """
...
class PipelineForkDemo1PythonTransformConfiguration(PythonTransformRuntimeConfiguration):
    """
    Implements the PythonTransformConfiguration for NOOP as required by the PythonTransformLauncher.
    NOOP does not use a RayRuntime class so the superclass only needs the base
    python-only configuration.
...
class PipelineForkDemoPythonTransformConfiguration(PythonTransformRuntimeConfiguration):
    """
    Implements the PythonTransformConfiguration for NOOP as required by the PythonTransformLauncher.
    NOOP does not use a RayRuntime class so the superclass only needs the base
    python-only configuration.
    ...
    def __init__(self):
        ...
        Initialization
        """
        pipeline1 = PipelineForkDemo1PythonTransformConfiguration()
        pipeline2 = PipelineForkDemo2PythonTransformConfiguration()
        super().__init__(
            transform_config=PipelineTransformConfiguration(
                pipeline=[
                    NOOPPythonTransformConfiguration(),
