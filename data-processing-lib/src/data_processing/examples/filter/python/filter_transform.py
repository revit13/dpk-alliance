...
class FilterTransform(AbstractTableTransform):
    ...
    def __init__(self, config: dict):
        ...
        if bool(self.input_arrow_folder.strip()):
            self.input_arrow_folder = self.input_arrow_folder if self.input_arrow_folder.endswith(
                "/") else f"{self.input_arrow_folder}/"
        self.output_arrow_folder = config.get(filter_output_arrow_folder_key, None)
        if bool(self.output_arrow_folder.strip()):
            self.output_arrow_folder = self.output_arrow_folder if self.output_arrow_folder.endswith(
                "/") else f"{self.output_arrow_folder}/"
        ...
        if self.parquet_input_folder is not None:
            self.parquet_input_folder = self.parquet_input_folder if self.parquet_input_folder.endswith(
    ...
    def _filter_arrow_and_meta_files(self, file_name: str, filtered_ids_list: list):
        ...
        try:
            ...
            input_arrow_tokens_batches = [arrow_reader.get_batch(i)["tokens"].to_pylist() for i in
                                          range(arrow_reader.num_record_batches)]
        ...
        output_ids_lines = [f"{doc_id}, {token_count}\n" for doc_id, token_count in
                            zip(filtered_ids_list, output_token_count_list)]
        ...
        try:
            ...
            output_arrow_record_batches = [pa.RecordBatch.from_arrays([array], schema=schema) for array in
                                           output_arrow_tokens_array]
...
class FilterTransformConfiguration(TransformConfiguration):
    ...
    def apply_input_params(self, args: argparse.Namespace) -> bool:
        ...
        logger.info(f"filter parameters are : {self.params}")
        return True