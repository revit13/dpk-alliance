...
from data_processing.examples.filter.python import FilterTransformConfiguration, Filter2TransformConfiguration
...
class Filter2SparkTransformConfiguration(SparkTransformRuntimeConfiguration):
    """
    Implements the SparkTransformConfiguration for NOOP as required by the PythonTransformLauncher.
    NOOP does not use a RayRuntime class so the superclass only needs the base
    python-only configuration.
    ...
    def __init__(self):
        ...
        Initialization
        """
        super().__init__(transform_config=Filter2TransformConfiguration())
