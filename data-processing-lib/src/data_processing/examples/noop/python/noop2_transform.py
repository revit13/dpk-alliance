...
class NOOP2TransformConfiguration(TransformConfiguration):
    """
    ...
    def apply_input_params(self, args: Namespace) -> bool:
        ...
        if captured.get(sleep_key) < 0:
            print(
                f"Parameter NOOP2_sleep_sec should be non-negative. you specified {args.noop_sleep_sec}"
            )
            return False

        self.params = self.params | captured
        logger.info(f"NOOP2 parameters are : {self.params}")
        return True


class NOOP2PythonTransformConfiguration(PythonTransformRuntimeConfiguration):
    """
    ...
    def __init__(self):
        ...
        Initialization
        """
        super().__init__(transform_config=NOOP2TransformConfiguration())


if __name__ == "__main__":
    launcher = PythonTransformLauncher(NOOP2PythonTransformConfiguration())
    logger.info("Launching NOOP2 transform")
    launcher.launch()
