...
class DataAccessFactoryBase(CLIArgumentProvider):
    ...
    def _validate_hf_config(self, hf_config: dict[str, str]) -> bool:
        ...
        if hf_config.get("hf_token", "") == "":
            self.logger.warning(
                f"data access factory {self.cli_arg_prefix}: " f"HF token is not defined, write operation may fail"
            )
        ...
        if input_folder is not None and not input_folder.startswith("datasets/"):
            valid_config = False
            self.logger.error(
                f"data access factory {self.cli_arg_prefix}: " f"Input folder in HF config has to start from datasets/"
            )
